"""Identical six-channel visual test for a 5x11 serpentine tooth module."""
...
def path_chase(channels):
    print("PATTERN 1: sequential 55-pixel path chase")
...
def column_sweep(channels):
    print("PATTERN 2: five-column sweep, left to right")
...
def row_sweep(channels):
    print("PATTERN 3: eleven-row sweep, top to bottom")
    for row in range(config.MODULE_HEIGHT):
        clear(channels)
        for column in range(config.MODULE_WIDTH):
...
print("\nBU-22 REV D 5x11 TOOTH MODULE TEST", VERSION)
...
print("Current-limit external 5 V; begin with only CH6 connected.")
